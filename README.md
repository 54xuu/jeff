...
- **定时任务**：到点自动向某个私聊/项目群发消息并让对方回复（每天 8 点问病区动态、订阅 AI 资讯早报）。5 段 cron、错过可配「开机补跑 / 跳过」、运行历史可查。见 [docs/schedules.md](docs/schedules.md)
- **插件**：把「必须用但不通用」的能力打包（如智慧病房），启用即自动接入其 MCP 工具（免手工配 MCP）+ 提供 `/` 快捷指令 + 用内置浏览器打开首页，并随 WebDAV 备份恢复。见 [docs/plugins.md](docs/plugins.md)
