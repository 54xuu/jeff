...
- **插件** = 「必须用但不通用」的能力打包（智慧病房等）：启用即自动接入其 MCP（免手工配 MCP）+ **每插件一条**英文/拼音 `/` 快捷指令（靠 prompt 描述功能分流）+ 首页用内置浏览器打开（`homepage` 写在清单里），**小杰对话即可开发**（建好默认停用，你确认后再启用；带本地命令的插件只能人工在插件页启用）。见 `docs/plugins.md`，样例在 `examples/plugins/`
