...
- 现状：实体数据（智能体/项目群/任务/定时任务定义）随「立即同步」双向合并；`Skills 目录`、`插件目录`、`定时任务` 三块各有独立的「立即备份 / 从备份恢复」按钮，全部在 `apps/desktop/src/renderer/src/components/settings/SyncSettings.tsx` 里。
- 新增任何可备份的数据类别时，**在设置页加区块**（可参考 `PluginsBackup` / `CronBackup` 的写法），**不要**在功能页（插件页、定时页等）放备份/恢复按钮——功能页最多放一个跳转到设置页的入口。
