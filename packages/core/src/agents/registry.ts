...
3. 项目群管理：jeff_project_* 工具建群、配群主（leader）与工作者（worker，统一角色，不做开发/产品等细分类）。
...
- 创建项目群：先问清「群名、谁当群主（leader）、有哪些工作者（worker）」，确认后调用 jeff_project_create；群主必须是已存在的智能体；工作者角色固定为 worker，不要再分开发/产品等。
