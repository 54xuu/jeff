...
export function allToolDefs(): ToolDef[] {
  return [
    ...
    {
      name: 'jeff_spawn_subtask',
      description:
        '把一项工作丢给一个全新的、空白的会话去独立完成，不会带上你当前对话里的任何历史——' +
        '用户不会说出工具名。当本轮系统提示里出现「【独立子任务】」，或用户用中文说了「每个文件独立处理 / 彼此独立 / 互不相关 / 独立子任务」时，必须使用本工具，不要改用 read 自己读原始文件。' +
        '如果目标之间其实有关联（同一份材料的不同章节、需要互相对比参考），不要用这个工具，直接在当前对话里处理。' +
        '子任务与你使用同一个身份（同模型、同指令、同工具集，含文件/命令工具），只是从零开始一段新历史；' +
        '如果任务要求产出文件，子任务会自己用文件工具写盘，不会把内容整段回传给你——你只会收到简短的完成状态，' +
        '这样处理很多个独立目标时你的对话不会被逐一的完整内容撑爆。请串行调用（等上一个结果回来再发下一个），' +
        '每条不同的用户消息内最多调用 80 次。子任务内部不能再调用本工具（不支持嵌套）。',
      args: {
        label: { type: 'string', description: '短标题（如「标书A-功能提取」），仅用于展示，不进子任务实际指令' },
        instruction: { type: 'string', description: '完整任务说明：做什么、依据什么材料、产出到哪里' },
        target_path: { type: 'string', description: '本次任务对应的目标文件/条目路径（可选，仅用于展示与去重提示，不做强校验）' },
      },
    },
