...
```jsonc
{
  ...
  // 且跨插件全局唯一。界面显示中文 name；选中后输入框出现「图标 + 中文名」筹码（只占自身宽度，换行从左边排），
...
3. `commands` 出现在私聊/群聊输入框的 `/` 菜单里（按组显示，这一轮只有「插件」组；每一行：`/{指令} {图标} {中文名} {简介}`）。选中后输入框出现筹码（图标 + 中文名，只占内容宽度），发给模型的是一句短约束 + 用户原话，**不会**把 `prompt` 整段塞进输入框。
