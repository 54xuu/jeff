...
import { api } from '../api'
import { IPC, type BrowserAction, type BrowserResult } from '@jeff/core'
import { registerBrowserHost, unregisterBrowserHost } from '../browserHost'
...
export default function BrowserPanel(): React.JSX.Element {
  ...
  const navigateTo = useCallback(
    async (url: string): Promise<{ url: string; title: string }> => {
      const wv = (await waitForView()) as unknown as { loadURL: (u: string) => Promise<void>; setAttribute: (k: string, v: string) => void; getURL: () => string; getTitle: () => string }
      try {
      ...
      return { url: wv.getURL() || url, title: wv.getTitle() || '' }
    },
    [waitForView],
  )
  ...
  useEffect(() => {
    ...
    const startUrl = browser.url || localStorage.getItem('jeff-browser-last-url') || ''
